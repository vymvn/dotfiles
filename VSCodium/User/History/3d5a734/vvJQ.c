...
char *iter_get_wstring(DBusMessageIter *iter) {
    int type = dbus_message_iter_get_arg_type(iter);
...
wchar_t *str_replace_all_wide(const wchar_t *str, const wchar_t *find, const wchar_t *repl) {
    // Pointer to substring of str
    const wchar_t *substr = str;

    // # of characters that need to be allocated per replacement
    const int REPL_DIFF = wcslen(repl) - wcslen(find);
    ...
    // character.
    size_t new_str_size = wcslen(str) + REPL_DIFF * REALLOC_RATE + 1;
...
wchar_t *str_trunc_wide(const wchar_t *wstr, const int max_len, const wchar_t *trunc) {
    const size_t len = wcslen(wstr);
    const size_t trunc_len = wcslen(trunc);
    wchar_t *new_wstr;
    ...
    if (len > max_len) {
        ...
        const size_t new_wstr_size = max_len + 1;

        new_wstr = (wchar_t *)calloc(new_wstr_size, sizeof(wchar_t));

        // Copy wstr, leaving room for trunc
        wcsncpy(new_wstr, wstr, max_len - trunc_len);
        wcscat(new_wstr, trunc);
    } else {
        // +1 for null char
        const size_t new_wstr_size = len + 1;
...
int num_of_matches_wide(const wchar_t *wstr, const wchar_t *find) {
    const wchar_t *substr = wstr;
    wchar_t *match;
