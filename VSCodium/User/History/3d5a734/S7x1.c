...
wchar_t *wstr_replace_all(const wchar_t *wstr, const wchar_t *find, const wchar_t *repl) {
    // Pointer to substring of wstr
    ...
    // character.
    size_t new_str_size = wcslen(wstr) + REPL_DIFF * REALLOC_RATE + 1;
...
wchar_t *wstr_trunc(const wchar_t *str, const int max_len, const wchar_t *trunc) {
    ...
    if (len > max_len) {
        ...
        wcsncpy(new_str, str, max_len - trunc_len);
        wcscat(new_str, trunc);
    } else {
        // +1 for null char
