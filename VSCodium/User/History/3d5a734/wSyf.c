...
wchar_t *iter_get_string(DBusMessageIter *iter) {
    ...
    if (type == DBUS_TYPE_STRING) {
        ...
        // +1 for null wchar_t
        size_t size = (strlen(value.str) + 1) * sizeof(wchar_t);
...
dbus_bool_t recurse_iter_of_signature(DBusMessageIter *iter,
    ...
    wchar_t *iter_signature = dbus_message_iter_get_signature(iter);
...
dbus_bool_t iter_go_to_key(DBusMessageIter *element_iter,
    ...
    wchar_t *iter_signature = dbus_message_iter_get_signature(element_iter);
...
wchar_t *join_path(const wchar_t *p1, const wchar_t *p2) {
    ...
    // Check if last wchar_tacter of p1 is '/'
    if (p1[len1 - 1] != '/') {
        // +1 for null wchar_t and +1 for '/'
        const size_t res_size = (len1 + len2 + 2) * sizeof(wchar_t);
        res = (wchar_t *)malloc(res_size);
        ...
        // +1 for null wchar_t
        const size_t res_size = (len1 + len2 + 1) * sizeof(wchar_t);
        res = (wchar_t *)malloc(res_size);
...
dbus_bool_t get_polybar_ipc_paths(const wchar_t *ipc_path, wchar_t **ptr_paths[],
    ...
    *num_of_paths = 3;
    // Allocate memory for num_of_paths wchar_t* pointers
    wchar_t **paths = (wchar_t **)malloc(*num_of_paths * sizeof(wchar_t *));
    ...
    if (d) {
        ...
        while ((dir = readdir(d)) != NULL) {
            const wchar_t *name = dir->d_name;

            // Check if filename starts with polybar_mqueue
            if (strncmp(name, "polybar_mqueue", 14) == 0) {
                // Join filename with parent path
                wchar_t *path = join_path(ipc_path, name);
                size_t len = strlen(path) + 1;  // +1 for null wchar_t

                if (i >= *num_of_paths) {
                    // Reallocate 3 additional paths
                    paths = (wchar_t **)realloc(paths, (i + 3) * sizeof(wchar_t *));
                }
        ...
        *num_of_paths = i;
        ...
        paths = (wchar_t **)realloc(paths, *num_of_paths * sizeof(wchar_t *));
...
wchar_t *str_replace_all(const wchar_t *str, const wchar_t *find, const wchar_t *repl) {
    ...
    const wchar_t *substr = str;

    // # of wchar_tacters that need to be allocated per replacement
    ...
    wchar_t *new_str = (wchar_t *)calloc(new_str_size, sizeof(wchar_t));
    ...
    while (match = strstr(substr, find)) {
        ...
        if (num_of_replacements != 0 &&
            ...
            new_str_size += REPL_DIFF * REALLOC_RATE * sizeof(wchar_t);
            new_str = (wchar_t *)realloc(new_str, new_str_size);
        }
        ...
        // Shift substr pointer to wchar_tacter after match
    ...
    new_str = (wchar_t *)realloc(new_str, (actual_len + 1) * sizeof(wchar_t));
...
wchar_t *str_trunc(const wchar_t *str, const int max_len, const wchar_t *trunc) {
    ...
    if (len > max_len) {
        // New size is max_len + null wchar_t
        const size_t new_str_size = max_len + 1;

        new_str = (wchar_t *)calloc(new_str_size, sizeof(wchar_t));
