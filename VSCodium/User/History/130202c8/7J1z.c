...
char *format_output(const char *artist, const char *title,
    ...
    const int ARTIST_REPL_DIFF = mbstowcs(NULL, artist, 0) - wcslen(L"%artist%");
    const int TITLE_REPL_DIFF = mbstowcs(NULL, title, 0) - wcslen(L"%title%");
    ...
    if (max_length == INT_MAX || TOTAL_UNTRUNC_LENGTH > max_length) {
        ...
        wchar_t *wtemp = str_replace_all_widechar(format, "%artist%", trunc_artist);
        wchar_t *wtemp2 = str_replace_all_widechar(wtemp, "%title%", trunc_title);
        output = str_replace_all(wtemp2, L"%", "");

        // Truncate output to max length
        if (!(output = str_trunc(output, max_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
                fputs(
        ...
        // Replace all tokens with their values
        wchar_t *wtemp = str_replace_all_widechar(format, "%artist%", artist);
        output = str_replace_all(wtemp, "%title%", title);
