...
wchar_t *format_output(const wchar_t *artist, const wchar_t *title,
    ...
    if (max_length == INT_MAX || TOTAL_UNTRUNC_LENGTH > max_length) {
        ...
        wchar_t *temp = str_replace_all(format, L"%artist%", trunc_artist);
        wchar_t *temp2 = str_replace_all(temp, L"%title%", trunc_title);
        ...
        // Replace all tokens with their values
        wchar_t *temp = str_replace_all(format, L"%artist%", artist);
        output = str_replace_all(temp, L"%title%", title);
