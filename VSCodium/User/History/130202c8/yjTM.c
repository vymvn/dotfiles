...
#include <stdlib.h>
#include <string.h>
#include <wchar.h>  // Include the <wchar.h> header file

#include "../include/utils.h"

/*************** Constants for DBus ***************/
const wchar_t *DESTINATION = L"org.mpris.MediaPlayer2.spotify";
const wchar_t *PATH = L"/org/mpris/MediaPlayer2";

const wchar_t *STATUS_IFACE = L"org.freedesktop.DBus.Properties";
const wchar_t *STATUS_METHOD = L"Get";
const wchar_t *STATUS_METHOD_ARG_IFACE_NAME = L"org.mpris.MediaPlayer2.Player";
const wchar_t *STATUS_METHOD_ARG_PROPERTY_NAME = L"Metadata";

const wchar_t *PLAYER_IFACE = L"org.mpris.MediaPlayer2.Player";
const wchar_t *PLAYER_METHOD_PLAY = L"Play";
const wchar_t *PLAYER_METHOD_PAUSE = L"Pause";
const wchar_t *PLAYER_METHOD_PLAYPAUSE = L"PlayPause";
const wchar_t *PLAYER_METHOD_NEXT = L"Next";
const wchar_t *PLAYER_METHOD_PREVIOUS = L"Previous";

const wchar_t *METADATA_TITLE_KEY = L"xesam:title";
const wchar_t *METADATA_ARTIST_KEY = L"xesam:artist";
...
wchar_t *get_song_title_from_metadata(DBusMessage *msg) {
    ...
    if (iter_try_step_into_type(&iter, DBUS_TYPE_VARIANT) &&
        ...
        title = iter_get_wstring(&iter);  // Use the wide character version of the function
    }
...
wchar_t *get_song_artist_from_metadata(DBusMessage *msg) {
    DBusMessageIter iter;
...
wchar_t *format_output(const wchar_t *artist, const wchar_t *title,
                        const int max_artist_length, const int max_title_length,
                        const int max_length, const wchar_t *format,
                        const wchar_t *trunc) {
    // Get total number of each token
    const int NUM_OF_ARTIST_TOK = num_of_matches_wide(format, L"%artist%");
    const int NUM_OF_TITLE_TOK = num_of_matches_wide(format, L"%title%");

    // Get length difference caused by a single replacement
    const int ARTIST_REPL_DIFF = wcslen(artist) - wcslen(L"%artist%");
    const int TITLE_REPL_DIFF = wcslen(title) - wcslen(L"%title%");

    // Calculate the total untruncated length of the output
    const int TOTAL_UNTRUNC_LENGTH = wcslen(format) +
    ...
    if (max_length == INT_MAX || TOTAL_UNTRUNC_LENGTH > max_length) {
        wchar_t *trunc_title;
        wchar_t *trunc_artist;

        // Truncate artist and track title using the truncation string
        if (!(trunc_title = str_trunc_wide(title, max_title_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
                fwprintf(stderr, L"Failed to truncate title. Please make sure the trunc "
                    L"string is smaller than the max title length.\n");
                exit(1);
            }
        }

        if (!(trunc_artist = str_trunc_wide(artist, max_artist_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
                fwprintf(stderr, L"Failed to truncate artist. Please make sure the trunc "
                    L"string is smaller than the max artist length.\n");
            }
            exit(1);
        }

        // Replace all tokens with their values
        wchar_t *temp = str_replace_all_wide(format, L"%artist%", trunc_artist);
        wchar_t *temp2 = str_replace_all_wide(temp, L"%title%", trunc_title);

        // Truncate output to max length
        if (!(output = str_trunc_wide(temp2, max_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
                fwprintf(stderr, L"Failed to truncate output. Please make sure the trunc "
                    L"string is smaller than the max output length.\n");
            }
            exit(1);
        }
        ...
        // Replace all tokens with their values
        wchar_t *temp = str_replace_all_wide(format, L"%artist%", artist);
        output = str_replace_all_wide(temp, L"%title%", title);
...
void get_status(DBusConnection *connection, const int max_artist_length,
                ...
                const wchar_t *format, const wchar_t *trunc) {
    DBusError err;
    dbus_error_init(&err);
    ...
    wchar_t *title = get_song_title_from_metadata(reply);
    wchar_t *artist = get_song_artist_from_metadata(reply);

    wchar_t *output = format_output(artist, title, max_artist_length,
...
void spotify_player_call(DBusConnection *connection, const wchar_t *method) {
    DBusError err;
    dbus_error_init(&err);
