...
wchar_t *format_output(const wchar_t *artist, const wchar_t *title,
    ...
    const int NUM_OF_ARTIST_TOK = num_of_matches(format, L"%artist%");
    const int NUM_OF_TITLE_TOK = num_of_matches(format, L"%title%");
    ...
    if (max_length == INT_MAX || TOTAL_UNTRUNC_LENGTH > max_length) {
        ...
        if (!(trunc_title = wstr_trunc(title, max_title_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
                fwprintf(stderr, L"Failed to truncate title. Please make sure the trunc string is smaller than the max title length.\n");
                exit(1);
            }
        }

        if (!(trunc_artist = wstr_trunc(artist, max_artist_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
                fwprintf(stderr, L"Failed to truncate artist. Please make sure the trunc string is smaller than the max artist length.\n");
            }
            exit(1);
        }

        // Replace all tokens with their values
        wchar_t *temp = wstr_replace_all(format, L"%artist%", trunc_artist);
        wchar_t *temp2 = wstr_replace_all(temp, L"%title%", trunc_title);

        // Truncate output to max length
        if (!(output = wstr_trunc(temp2, max_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
                fwprintf(stderr, L"Failed to truncate output. Please make sure the trunc string is smaller than the max output length.\n");
            }
            exit(1);
        }
        ...
        // Replace all tokens with their values
        wchar_t *temp = wstr_replace_all(format, L"%artist%", artist);
        output = wstr_replace_all(temp, L"%title%", title);
