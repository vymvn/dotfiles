...
wchar_t *format_output(const wchar_t *artist, const wchar_t *title,
    ...
    const int NUM_OF_ARTIST_TOK = num_of_matches_w(format, L"%artist%");
    const int NUM_OF_TITLE_TOK = num_of_matches_w(format, L"%title%");
    ...
    if (max_length == INT_MAX || TOTAL_UNTRUNC_LENGTH > max_length) {
        ...
        if (!(trunc_title = str_trunc_w(title, max_title_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
        ...
        if (!(trunc_artist = str_trunc_w(artist, max_artist_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
        ...
        wchar_t *temp = str_replace_all_w(format, L"%artist%", trunc_artist);
        wchar_t *temp2 = str_replace_all_w(temp, L"%title%", trunc_title);

        // Truncate output to max length
        if (!(output = str_trunc_w(temp2, max_length, trunc))) {
            if (!SUPPRESS_ERRORS) {
        ...
        // Replace all tokens with their values
        wchar_t *temp = str_replace_all_w(format, L"%artist%", artist);
        output = str_replace_all_w(temp, L"%title%", title);
